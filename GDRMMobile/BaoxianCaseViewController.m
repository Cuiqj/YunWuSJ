...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    ...
    if (textField ==self.textStationStartKM) {
        ...
        if([self.roadSegmentID isEqualToString:@"666666666"]){
            ...
            self.textStationEndKM.text=[NSString stringWithFormat:@"%03d",iShoufz.station_end.integerValue/1000];
...
//        self.roadSegmentID      = self.caseInfo.roadsegment_id;
        ...
        self.textStationEndKM.text=[NSString stringWithFormat:@"%02d",self.caseInfo.station_end.integerValue/1000];
        self.textStationEndM.text=[NSString stringWithFormat:@"%03d",self.caseInfo.station_end.integerValue%1000];
...
//    self.caseInfo.roadsegment_id  = self.roadSegmentID ;//[NSString stringWithFormat:@"%d", [self.roadSegmentID intValue]];
    ...
    self.textStationEndM.text=[NSString stringWithFormat:@"%03d",self.caseInfo.station_end.integerValue%1000];
    if (self.caseInfo.station_start.integerValue == 0 && self.caseInfo.station_end.integerValue == 0) {
        [self.SwitchNumber setOn:NO];
        [self SwitchOnOrOff:nil];
    }
    NSDateFormatter *formatter=[[NSDateFormatter alloc] init];
    [formatter setLocale:[NSLocale currentLocale]];
    [formatter setDateFormat:@"yyyy-MM-dd HH:mm"];
...
- (void)saveCaseProveInfoForCase:(NSString *)caseID{
    ...
    if (station_start.integerValue == 0 || station_start.integerValue == station_end.integerValue  ) {
        stationString=[NSString stringWithFormat:@"K%02d+%03dM",self.textStationStartKM.text.integerValue,self.textStationStartM.text.integerValue];
    } else {
        stationString=[NSString stringWithFormat:@"K%02d+%03dM至K%02d+%03dM",self.textStationStartKM.text.integerValue,self.textStationStartM.text.integerValue,self.textStationEndKM.text.integerValue,self.textStationEndM.text.integerValue];
    }
    //以前的勘验场所描述
...
//        caseProveInfo.remark = [NSString stringWithFormat:@"%@%@%@%@",self.textRoadSegment.text,self.textSide.text,stationString,self.textPlace.text];
    ...
    NSArray * array =  [[NSUserDefaults standardUserDefaults] objectForKey:@"INSPECTORARRAYKEY"];
    if ([array count]>=2) {
        caseProveInfo.prover = [NSString stringWithFormat:@"%@、%@",array[0],array[1]];
    }else{
