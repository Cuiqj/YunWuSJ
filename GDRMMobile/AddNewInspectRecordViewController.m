...
- (void)viewDidLoad
{
    ...
    self.textStationEndKM.tag = 334;
    self.textStationEndM.tag = 335;
    ...
    if (self.descState == kAddNewRecord) {
        ...
        if (!isBlank) {
            ...
            if([self.textStationEndKM.text isEmpty]&&[self.textStationEndM.text isEmpty]){
                remark=[[NSString alloc] initWithFormat:@"%@巡至%@%@K%02ld+%03ldM处时，在公路%@%@，巡逻班组%@。",timeString,self.textSegement.text,self.textSide.text,self.textStationStartKM.text.integerValue,self.textStationStartM.text.integerValue,self.textPlace.text,self.textCheckReason.text,self.textCheckStatus.text];
            }
            else{
                remark=[[NSString alloc] initWithFormat:@"%@巡至%@%@K%02ld+%03ldM与K%02ld+%03ldM之间时，在公路%@%@，巡逻班组%@。",timeString,self.textSegement.text,self.textSide.text,self.textStationStartKM.text.integerValue,self.textStationStartM.text.integerValue,self.textStationEndKM.text.integerValue,self.textStationEndM.text.integerValue,self.textPlace.text,self.textCheckReason.text,self.textCheckStatus.text];
            }
        ...
        if (!isBlank) {
            ...
            if ([self.textViewNormalDesc.text isEmpty]) {
                self.textViewNormalDesc.text = [NSString stringWithFormat:@"%@巡查%@%@，%@",time,self.textRoad.text,self.textPlaceNormal.text,self.textDescNormal.text];
            }
            InspectionRecord *inspectionRecord=[InspectionRecord newDataObjectWithEntityName:@"InspectionRecord"];
            inspectionRecord.roadsegment_id=[NSString stringWithFormat:@"%d", [self.roadSegmentID intValue]];
...
- (void)setBridge:(NSString *)aBridgeID roadName:(NSString *)raBridgeName stake_mark:(NSString *)stake_mark{
    self.bridgeID            = aBridgeID;
    if (aBridgeID.length <=0) {
        self.bridgeID = @"0";
    }
    self.textBridgeName.text = [NSString stringWithFormat:@"%@%@",raBridgeName,stake_mark];
...
- (IBAction)UNderBridgeClickname:(id)sender {
    
    [self.delegate addObserverToKeyBoard];
    [self dismissViewControllerAnimated:YES completion:^{
        [self.delegate toUnderBridgeView:self.textBridgeName.text];
