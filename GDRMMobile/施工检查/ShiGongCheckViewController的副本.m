...
//    if(self.textchecker.text.length>0)
...
//    self.maintainPlanID = self.constructionID;          //施工检查上传问题  这个表不上传
...
//    checkInfo.check_project_id=  self.maintainPlanID;
//    checkInfo.check_danwei = self.textShiGongDanWei.text;
...
//    cell.textLabel.text=[formatter stringFromDate:constructionInfo.check_date];
    cell.textLabel.text = [NSString stringWithFormat:@"%@  %@",[MaintainPlan maintainPlanNameForID:constructionInfo.maintainPlan_id],[formatter stringFromDate:constructionInfo.check_date]];
     ...
     */
    cell.detailTextLabel.text = constructionInfo.jiancha_didian;
    cell.textLabel.backgroundColor=[UIColor clearColor];
...
- (IBAction)BtnClickRemoveUserData:(id)sender {
