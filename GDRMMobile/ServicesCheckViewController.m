...
@property (nonatomic,strong) UIImageView * imageview;       //显示签名
@end
...
- (void)viewDidLoad {
    ...
    serviceCheckIDaddNew = @"addNew";
}
- (void)imagebuttonClick:(UIButton *)button{
    BOOL addNew = serviceCheckIDaddNew.length >0;
    if(serviceCheckID.length >0 && !addNew) {
        ...
        UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"提示" message:@"请先保存数据，再签名"  preferredStyle:UIAlertControllerStyleAlert];
...
- (void) viewWillDisappear:(BOOL)animated{
    if (serviceCheckID !=nil && self.roadinspectVC ) {
...
- (IBAction)btnSave:(id)sender {
    BOOL textemtmp = self.textchecktime.text.length && self.textservicename.text.length && self.textservice_fuzeren.text.length && self.textcheck_fuzeren.text.length;
    if (!textemtmp) {
        __weak typeof(self)weakself = self;
        UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"提示" message:@"请先填写数据，再保存"  preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            return ;
        }];
        [ac addAction:cancelAction];
...
- (IBAction)BtnClickRemoveNameData:(id)sender {
    self.textcheck_fuzeren.text = @"";
    self.serviceCheck.check_fuzeren = @"";
}
- (void) setSelectData:(NSString *)data{
    switch (textTagFlag) {
        ...
        case kCHECKFUZEREN:{
            if ([self.textcheck_fuzeren.text length]>0) {
                self.textcheck_fuzeren.text = [NSString stringWithFormat:@"%@、%@",self.textcheck_fuzeren.text,data];
            }else{
...
//    NSString * filePath=[Path stringByAppendingPathComponent:Name];
    ...
    self.imageview.frame = CGRectMake(frame.origin.x -80, frame.origin.y+60, image.size.width, image.size.height);
    self.imageview.tag = 555;
    [self.serviceCheckView addSubview:self.imageview];
}
