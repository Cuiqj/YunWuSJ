...
- (void)viewDidLoad
{
    switch (self.pickerState) {
        case kCheckHandle:
            self.data=[[CheckHandle handleForCheckType:self.checkTypeID] valueForKey:@"handle_name"];//处理结果
            break;
        case kCheckReason:
            self.data=[[CheckReason reasonForCheckType:self.checkTypeID] valueForKey:@"reasonname"];//类型明细
            break;
        case kCheckStatus:
            self.data=[[CheckStatus statusForCheckType:self.checkTypeID] valueForKey:@"statusname"];//处理措施
            break;
        case kCheckType:
            self.data=[CheckType allCheckType];   //事故类型
            break;
        case kWeatherPicker:
