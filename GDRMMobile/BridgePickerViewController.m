...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.pickerState==kBridgeName) {
        NSString * name_num = [NSString stringWithFormat:@"%@%@",[[self.data objectAtIndex:indexPath.row] valueForKey:@"name"],[[self.data objectAtIndex:indexPath.row] valueForKey:@"stake_mark"]];
        cell.textLabel.text=name_num;
    } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (self.pickerState) {
        case kBridgeName:
            [self.delegate setBridge: [[self.data objectAtIndex:indexPath.row] valueForKey:@"myid"] roadName: [[self.data objectAtIndex:indexPath.row] valueForKey:@"name"] stake_mark:[[self.data objectAtIndex:indexPath.row] valueForKey:@"stake_mark"]];
            break;
        case kBridgeDesc:
