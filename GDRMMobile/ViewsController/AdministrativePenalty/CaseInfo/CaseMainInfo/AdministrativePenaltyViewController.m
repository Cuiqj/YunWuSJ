...
- (IBAction)textfieldClick:(id)sender {
    if (self.listpopover && [self.listpopover isPopoverVisible]) {
    ...
    listPicker.data     = @[@"已移交综合执法局",@"已制止"];
    self.listpopover=[[UIPopoverController alloc] initWithContentViewController:listPicker];
    CGRect rect         = textfield.frame;
    [self.listpopover presentPopoverFromRect:rect inView:self.view permittedArrowDirections:UIPopoverArrowDirectionLeft animated:YES];
    listPicker.pickerPopover = self.listpopover;
}
...
- (void)saveCaseInfoForCase:(NSString *)caseID{
    ...
    self.textStationStartKM.text=[NSString stringWithFormat:@"%02d", self.caseInfo.station_start.integerValue/1000];
    self.textStationStartM.text=[NSString stringWithFormat:@"%d03",self.caseInfo.station_start.integerValue%1000];
    self.textStationEndKM.text=[NSString stringWithFormat:@"%02d",self.caseInfo.station_end.integerValue/1000];
    self.textStationEndM.text=[NSString stringWithFormat:@"%03d",self.caseInfo.station_end.integerValue%1000];
    if (self.caseInfo.station_start.integerValue == 0 && self.caseInfo.station_end.integerValue == 0) {
