...
//            caseDescString=[caseDescString stringByAppendingFormat:@"    当事人%@于%@驾驶%@%@行驶至%@%@%@在公路%@由于%@发生交通事故损坏公路路产，现场%@，经与当事人现场勘查，",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseInfo.place,caseInfo.case_reason,caseStatusString];
            caseDescString=[caseDescString stringByAppendingFormat:@"    %@于%@驾驶%@%@行至%@%@%@，因发生交通事故导致损坏公路路产，%@，经与当事人现场勘查，",citizen.party,happenDate,citizen.automobile_number,citizen.automobile_pattern,roadName,caseInfo.side,stationString,caseStatusString];
...
//            caseDescString=[caseDescString stringByAppendingFormat:@"在公路%@由于%@发生碰撞，造成交通事故，现场%@，经与当事人现场勘查，",caseInfo.place,caseInfo.case_reason,caseStatusString];
            caseDescString=[caseDescString stringByAppendingFormat:@"因%@发生交通事故导致损坏公路路产，%@，经与当事人现场勘查，",caseStatusString];
...
- (NSString *) prover1{
    NSArray * chunks;
    if([self.prover containsString:@","]){
        chunks = [self.prover componentsSeparatedByString: @","];
    }if([self.prover containsString:@"、"]){
        chunks = [self.prover componentsSeparatedByString: @"、"];
    }
    if([chunks count]==2)
...
+ (NSString *)generateWoundDesc:(NSString *)caseID{
    ...
    caseStatusString = [NSString stringWithFormat:@"%@。",caseStatusString];
